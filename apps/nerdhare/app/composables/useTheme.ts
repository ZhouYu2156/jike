...
import { cssVariableDefaults, themeIcons } from "../utils/theme";
...
export function useTheme() {
  ...
  const cssVariablesData = useState<{
    ...
    readLocalStorage("nuxt-ui-css-variables", {}),
  );
  ...
  function exportCSS(): string {
    ...
    if (_font.value !== "Public Sans") {
      lines.push(
        "",
        "@theme {",
        `  --font-sans: '${_font.value}', sans-serif;`,
        "}",
      );
    }
    ...
    const lightOverrides = Object.entries(
      cssVariablesData.value.light || {},
    ).filter(
      ([key, val]) =>
        val !==
        cssVariableDefaults.light[
          key as keyof typeof cssVariableDefaults.light
        ],
    );
    const darkOverrides = Object.entries(
      cssVariablesData.value.dark || {},
    ).filter(
      ([key, val]) =>
        val !==
        cssVariableDefaults.dark[key as keyof typeof cssVariableDefaults.dark],
    );
