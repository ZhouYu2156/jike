...
![nodejs官网](/pictures/nodejs安装/nodejs官网.png)
...
![设置nodejs安装目录权限](/pictures/nodejs安装/设置nodejs安装目录权限.png)
...
![设置NODE_PATH系统环境变量](/pictures/nodejs安装/设置NODE_PATH变量.png)

> 继续设置`系统变量`中的`PATH`变量，在`变量值`中添加`E:\enviroments\nodejs\`和`%NODE_PATH%`。

![新增PATH路径和NODE_PATH系统路径](/pictures/nodejs安装/新增PATH系统路径.png)
...
![将要修改的路径](/pictures/nodejs安装/将要修改的路径.png)

修改如下，注意是`用户变量`块下的`PATH`变量，而不是`系统变量`块下的`PATH`变量。

![修改后的路径](/pictures/nodejs安装/设置npm全局安装路径的值.png)
...
![全局安装包检查配置是否生效](/pictures/nodejs安装/全局安装包检查配置是否生效.png)
